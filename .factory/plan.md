...
- [x] T4 (standard) — attribution: a non-UTF-8 filename freezes aggregation via the watermark/session-fallback binds (follow-up to #16) (Fixes #23)
  ...
  - pr: 29
- [R] T5 (standard) — poller: run attribution aggregation concurrently with the fetch so the first scan does not delay the usage meter (follow-up to #16) (Fixes #22)
